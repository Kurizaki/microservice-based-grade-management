    ...
    if (calcForm) {
        (async () => {
            ...
            try {
                ...
                const gradesResponse = await fetch(`${GRADE_API_BASE}/GetGradesFromUser`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    },
                ...
                const calcResponse = await fetch(`${CALC_API_BASE}/calculate`, {
                    ...
                    body: JSON.stringify(grades.map((grade) => ({
                ...
                if (calcResponse.ok) {
                        ...
                        .map(([category, averageGrade]) => {
                            ...
                            return `
                                <div class="category-box" id="category-${category.replace(/\s+/g, '-')}">
                                    ...
                                    ${individualGrades}
                                    <p class="category-average">Average: ${averageGrade.toFixed(2)}</p>
                                </div>
                            `;
                        })
                        .join("");
                    ...
                    // show final grade
                    document.getElementById("final-grade").value = result.finalGrade.toFixed(2);
